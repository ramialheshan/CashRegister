...
namespace CashRegister
{
    public partial class AssortmentOfGoodsForm : Form
    {
                ...
                case true:
                    ...
                    switch ((sender as RadioButton).Name)
                    {
                        ...
                        case "ExistingProductRadioButton":
                            ...
                            NumberForADDTextBox.Text = string.Empty;

                            NumberUnitProductComboBox.Enabled = true;
                            NumberUnitProductComboBox.SelectedIndex = -1;
                            NumberUnitProductComboBox.Enabled = false;

                            CostPerOneOfProductTextBox.Enabled = true;
                            CostPerOneOfProductTextBox.Text = string.Empty;
                            CostPerOneOfProductTextBox.Enabled = false;

                            ProductAllValueTextBox.Enabled = true;
                            ProductAllValueTextBox.Text = string.Empty;
        ...
        private void AddValueProductButton_Click(object sender, EventArgs e)
        {
            ...
            switch (ExistingProductRadioButton.Checked)
            {
                case true:
                    ...
                    if (string.IsNullOrEmpty(ProductComboBox.Text.Trim()) || ProductComboBox.Text == "Начните вводить название товара")
                    { MessageBox.Show("Введите название товара", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Exclamation); return; }
        ...
        private void RemoveValueProductButton_Click(object sender, EventArgs e)
        {
            ...
            #region Проверка наличия товара 

            bool find = false;
            if (Product_DataSet.Tables["Product_List_Of_Goods_Table"].Rows.Count > 0 && !string.IsNullOrEmpty(ProductForRemoveComboBox.Text.Trim()))
            {
                for (int i = 0; i < Product_DataSet.Tables["Product_List_Of_Goods_Table"].Rows.Count; i++)
                {
                    if (Product_DataSet.Tables["Product_List_Of_Goods_Table"].Rows[i]["Product_Name"].ToString() == ProductForRemoveComboBox.Text.Trim())
                    {
                        find = true; break;
                    }
                }
            ...
            if (!find) { MessageBox.Show("Введите название товара", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Exclamation); return;  }
                    ...
                    if (CurrentInfoForRemoveToCheck["Product_Name"].ToString() == Product_DataSet.Tables["Product_List_Of_Goods_Table"].Rows[i]["Product_Name"].ToString())
                    {
                        ...
                        switch (NumberUnitProduct_REMOVE_ComboBox.SelectedIndex)
                        {
                            ...
                            case 1:
                                ...
                                { 
                                    NumberFor_REMOVE_TextBox.Text = string.Empty;

                                    #region Очистка товара 
        ...
        #region Событие: При выборе вкладки с операцией 

        private void GoodsActionTabControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (GoodsActionTabControl.SelectedTab.Name)
            {
                case "AddGoodsTabPage":

                    NewProductRadioButton.Checked = false;
                    ExistingProductRadioButton.Checked = false;
                    NewProductRadioButton.Checked = true;
                    ProductAllValueTextBox.Enabled = true;
                    ProductAllValueTextBox.Text = string.Empty;
                    ProductAllValueTextBox.Enabled = false;
                    break;
                case "WriteOffGoodsTabPage":
                    ...
                    NumberUnitProduct_REMOVE_ComboBox.Enabled = true;
                    NumberUnitProduct_REMOVE_ComboBox.SelectedIndex = -1;
                    NumberUnitProduct_REMOVE_ComboBox.Enabled = false;

                    CostPerOneOfProduct_REMOVE_TextBox.Enabled = true;
                    CostPerOneOfProduct_REMOVE_TextBox.Text = string.Empty;
                    CostPerOneOfProduct_REMOVE_TextBox.Enabled = false;

                    ProductAllValue_REMOVE_TextBox.Enabled = true;
                    ProductAllValue_REMOVE_TextBox.Text = string.Empty;
                    ProductAllValue_REMOVE_TextBox.Enabled = false;
